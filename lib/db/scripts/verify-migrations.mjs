...
import pg from "pg";
import { sha256Hex } from "./manual-migrations/checksum.mjs";
import { verifyMigrationPostconditions } from "./manual-migrations/postconditions.mjs";
...
const toPrefix = process.env.MIGRATE_TO_PREFIX ?? "9999";
const mode = (process.env.MIGRATE_MODE ?? "apply").toLowerCase();
const isVerifyOnly = mode === "verify" || mode === "dry-run" || mode === "dryrun";
...
const getChecksum = async (client, tag) => {
  const r = await client.query(
...
const execFile = async (client, fileName) => {
  ...
  try {
    ...
    await client.query(
      "insert into public.lawcaspro_manual_migration_checksums(tag, checksum_sha256) values ($1, $2) on conflict do nothing",
      [tag, checksum],
    );
    await client.query("commit");
  } catch (err) {
    ...
    if (isSkippableAlreadyAppliedError(err)) {
      ...
      if (verify.ok) {
        await markApplied(client, tag);
        return { tag, status: "verified_present_legacy", issues: verify.issues };
      }
      throw Object.assign(new Error(`migration has duplicate-object error but postconditions are not satisfied: ${tag}`), {
        tag,
        fileName,
  ...
  try {
    if (isVerifyOnly) {
      const hasMigrationsTable = Boolean(
        await queryValue(client, "select to_regclass($1) as name", ["public.lawcaspro_manual_migrations"]),
      );
      const hasChecksumsTable = Boolean(
        await queryValue(client, "select to_regclass($1) as name", ["public.lawcaspro_manual_migration_checksums"]),
      );
      ...
      for (let i = startIdx; i <= endIdx; i++) {
        ...
        const applied = hasMigrationsTable ? await wasApplied(client, tag) : false;
        if (applied) {
          if (!hasChecksumsTable) {
            summary.push({ tag, status: "legacy_applied_without_checksum", fileName, note: "checksum_tracking_not_initialized" });
            continue;
          }
          const recorded = await getChecksum(client, tag);
          if (!recorded) {
            summary.push({ tag, status: "legacy_applied_without_checksum", fileName });
          } else if (recorded === checksum) {
            summary.push({ tag, status: "applied_checksum_matches", fileName });
          } else {
            summary.push({ tag, status: "checksum_mismatch", fileName });
          }
          continue;
        }
        ...
        if (verify.ok) {
          summary.push({
            tag,
            status: "schema_already_present_and_verified",
            fileName,
            ...(hasMigrationsTable ? {} : { note: "migration_history_not_initialized" }),
          });
        } else if (verify.issues?.[0] === "no_postcondition_verifier") {
          summary.push({
            tag,
            status: "pending",
            fileName,
      ...
      process.stdout.write(JSON.stringify({ ok: true, mode: "verify", summary }, null, 2));
