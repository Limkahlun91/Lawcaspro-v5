...
import { ApiError, getApiMeta, sendError, sendOk } from "../lib/api-response.js";
import {
...
function toCustomVariableApiError(
  err: unknown,
  opts: {
    stage: string;
    action: "create" | "update" | "preview" | "list";
    key?: string | null;
  },
): ApiError {
  if (getPgCode(err) === "23505" && opts.key) {
    return new ApiError({
      status: 409,
      code: "CUSTOM_VARIABLE_KEY_EXISTS",
      message: `A custom variable with the key '${opts.key}' already exists.`,
      retryable: false,
      details: { field: "key", key: opts.key },
      stage: opts.stage,
    });
  }
  if (isUndefinedTableError(err) || isUndefinedColumnError(err)) {
    return new ApiError({
      ...
      message: "Database schema is outdated. Apply migration 0113_document_custom_variables_and_variable_governance.sql.",
      retryable: false,
      stage: opts.stage,
    });
  }
  ...
  return new ApiError({
    ...
    message:
      opts.action === "preview"
        ? "Unable to preview the custom variable."
        : opts.action === "create"
          ? "Unable to save the custom variable."
          : opts.action === "update"
            ? "Unable to update the custom variable."
            : "Unable to load custom variables.",
    retryable: true,
    stage: opts.stage,
  });
}
...
async function writeDocumentGenerationLog(
  ...
  try {
    ...
    let usedFallback: "tier1" | "tier2" | "tier3" = "tier1";
    ...
    const logId =
      rows && typeof (rows[0] as any)?.id === "number"
...
async function buildCaseContext(
  ...
  return applyCaseVariableAliases({
    ...
    purchasers_inline: joinNamesWithAmpersand(purchaserNames),
...
router.get(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    try {
      ...
      logger.error({ err, route: req.path, userId: req.userId ?? null, requestId: (res.locals as any)?.requestId ?? null }, "platform_custom_variables.list_failed");
      sendError(res as any, toCustomVariableApiError(err, { stage: "platform_custom_variables.list", action: "list" }));
    }
  },
);
    ...
    try {
      const created = await withAuthSafeDb(async (authDb) => {
        ...
        if (sysVar[0]) {
          throw new ApiError({
            status: 409,
            code: "KEY_CONFLICT_SYSTEM",
            message: "Key conflicts with system variable",
            retryable: false,
            details: { field: "key" },
      ...
      logger.error({ err, route: req.path, userId: req.userId ?? null, requestId: (res.locals as any)?.requestId ?? null }, "platform_custom_variables.create_failed");
      sendError(
        res as any,
        err instanceof ApiError
          ? err
          : toCustomVariableApiError(err, { stage: "platform_custom_variables.create", action: "create", key: v.key }),
      );
    }
  },
);
...
router.put(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    try {
      const out = await withAuthSafeDb(async (authDb) => {
        ...
        if (!existing) {
          throw new ApiError({
            status: 404,
            code: "NOT_FOUND",
            message: "Custom variable not found",
            retryable: false,
            stage: "platform_custom_variables.update",
          });
        }
      ...
      logger.error({ err, route: req.path, userId: req.userId ?? null, requestId: (res.locals as any)?.requestId ?? null }, "platform_custom_variables.update_failed");
      sendError(
        res as any,
        err instanceof ApiError
          ? err
          : toCustomVariableApiError(err, { stage: "platform_custom_variables.update", action: "update" }),
      );
    }
  },
);
...
router.get(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    if (Number.isNaN(id) || Number.isNaN(firmId) || Number.isNaN(caseId)) {
      sendError(res as any, new ApiError({ status: 400, code: "INVALID_INPUT", message: "Invalid params", retryable: false, stage: "platform_custom_variables.preview" }));
      return;
    }
    try {
      ...
      if (!cv) {
        sendError(res as any, new ApiError({ status: 404, code: "NOT_FOUND", message: "Custom variable not found", retryable: false, stage: "platform_custom_variables.preview" }));
        return;
      }
      ...
      try {
        const out = await withFounderFirmRlsDb(req, firmId, async (r) => {
          ...
          if (!context) {
            throw new ApiError({
              ...
              retryable: false,
              stage: "platform_custom_variables.preview",
            });
          }
        ...
        sendError(res as any, new ApiError({ status: 403, code: "SUPPORT_SESSION_REQUIRED", message: "Support session required", retryable: false, stage: "platform_custom_variables.preview" }));
      }
    } catch (err) {
      logger.error({ err, route: req.path, userId: req.userId ?? null, requestId: (res.locals as any)?.requestId ?? null }, "platform_custom_variables.preview_failed");
      sendError(
        res as any,
        err instanceof ApiError
          ? err
          : toCustomVariableApiError(err, { stage: "platform_custom_variables.preview", action: "preview" }),
      );
    }
  },
);
...
router.get(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    try {
      ...
      logger.error({ err, route: req.path, firmId: req.firmId ?? null, userId: req.userId ?? null, requestId: (res.locals as any)?.requestId ?? null }, "documents_custom_variables.list_failed");
      sendError(res as any, toCustomVariableApiError(err, { stage: "documents_custom_variables.list", action: "list" }));
    }
  },
);
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    try {
      ...
      try {
        ...
        sendError(res as any, toCustomVariableApiError(err, { stage: "documents_custom_variables.create", action: "create", key: v.key }));
        return;
      }
...
router.put(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    try {
      ...
      const existing = rows[0] as any;
      if (!existing) {
        sendError(res as any, new ApiError({ status: 404, code: "NOT_FOUND", message: "Custom variable not found", retryable: false, stage: "documents_custom_variables.update" }));
        return;
      }
      ...
      logger.error({ err, route: req.path, firmId: req.firmId ?? null, userId: req.userId ?? null, requestId: (res.locals as any)?.requestId ?? null }, "documents_custom_variables.update_failed");
      sendError(res as any, toCustomVariableApiError(err, { stage: "documents_custom_variables.update", action: "update" }));
    }
  },
);
...
router.get(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    const id = idStr ? parseInt(idStr, 10) : NaN;
    if (Number.isNaN(id)) {
      sendError(res as any, new ApiError({ status: 400, code: "INVALID_INPUT", message: "Invalid ID", retryable: false, stage: "documents_custom_variables.preview" }));
      return;
    }
    const caseIdRaw = one((req.query as any).caseId);
    const caseId = caseIdRaw ? parseInt(String(caseIdRaw), 10) : NaN;
    ...
    try {
      ...
      if (!cv) {
        sendError(res as any, new ApiError({ status: 404, code: "NOT_FOUND", message: "Custom variable not found", retryable: false, stage: "documents_custom_variables.preview" }));
        return;
      }
      ...
      if (!caseContext) {
        sendError(res as any, new ApiError({ status: 404, code: "NOT_FOUND", message: "Case not found", retryable: false, stage: "documents_custom_variables.preview" }));
        return;
      }
      const baseResolved = resolveVariablesForTemplate({
        registry: registryAll,
      ...
      logger.error({ err, route: req.path, firmId: req.firmId ?? null, userId: req.userId ?? null, requestId: (res.locals as any)?.requestId ?? null }, "documents_custom_variables.preview_failed");
      sendError(res as any, toCustomVariableApiError(err, { stage: "documents_custom_variables.preview", action: "preview" }));
    }
  },
);
...
router.get(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    const hasTechnicalAuditPermission = await (async (): Promise<boolean> => {
      if (req.userType === "founder") return true;
      const firmId = typeof req.firmId === "number" ? req.firmId : null;
      const roleId = typeof req.roleId === "number" ? req.roleId : null;
      if (!firmId || !roleId) return false;
      try {
        const roleRows = await queryRows(r, sql`SELECT name FROM roles WHERE id = ${roleId} AND firm_id = ${firmId} LIMIT 1`);
        const name = typeof roleRows[0]?.name === "string" ? String(roleRows[0].name).toLowerCase() : "";
        if (name.includes("partner") || name.includes("admin") || name.includes("founder")) return true;
        const permRows = await queryRows(r, sql`
    ...
    const items = rows.map((row) => ({
      ...
      copiesConfigured:
        typeof (row as any).copies_configured === "number"
          ...
          : null,
      ipAddress:
        hasTechnicalAuditPermission && typeof (row as any).ip_address === "string"
          ? String((row as any).ip_address)
          : null,
      userAgent:
        hasTechnicalAuditPermission && typeof (row as any).user_agent === "string"
...
async function computeDocGenJobProgress(
  r: DbConn,
  args: { firmId: number; jobId: string },
  ...
  const rows = await queryRows(
    ...
    sql`
      SELECT
        COUNT(*) AS total,
        COUNT(*) FILTER (WHERE status IN ('success','succeeded')) AS success,
        COUNT(*) FILTER (WHERE status = 'failed') AS failed,
        COUNT(*) FILTER (WHERE status = 'pending') AS pending,
        COUNT(*) FILTER (WHERE status IN ('running','processing')) AS running
      FROM document_generation_job_items
      WHERE job_id = ${args.jobId} AND firm_id = ${args.firmId}
    `,
  );
...
async function finalizeDocGenJobIfDone(
  r: DbConn,
  args: { firmId: number; jobId: string },
): Promise<{
  finalized: boolean;
  ...
  if (!done) return { finalized: false, status: "running", progress };
...
async function ensureDocGenJobDownloadObject(
  ...
  if (existingPath) {
    return {
      downloadObjectPath: existingPath,
      downloadFileName:
        typeof job.download_file_name === "string"
          ? String(job.download_file_name)
          : action === "print"
  ...
  const items = await queryRows(
    ...
    sql`
      SELECT
        ...
        c.reference_no AS case_reference_no
      ...
      LEFT JOIN cases c ON c.id = i.case_id AND c.firm_id = i.firm_id
      WHERE i.job_id = ${args.jobId} AND i.firm_id = ${args.firmId}
      ORDER BY i.id ASC
    `,
  );
  ...
  const caseIds = Array.isArray(caseIdsRaw)
    ? caseIdsRaw
        ...
        .filter((x) => x > 0)
    : [];
  const persistDownloadFailure = (
    zipErrorCode: DocGenErrorCode,
    ...
    try {
      queryRows(
        ...
        sql`
          UPDATE document_generation_jobs
          SET status = 'generated_download_failed',
              error_code = ${zipErrorCode},
              error_summary = ${zipErrorMessage},
    ...
    try {
      logger.warn(
        {
          event: "document_generation_download_failed",
          firmId: args.firmId,
          jobId: args.jobId,
          userId,
          action,
        ...
        "[documents] generated_download_failed persisted",
      );
    } catch {}
    try {
      writeDocumentGenerationLog(r, {
        firmId: args.firmId,
        userId,
  ...
  if (action === "print") {
    try {
      ...
      if (!merged.length) {
        persistDownloadFailure("OUTPUT_MISSING", "Merged print PDF output was empty.");
        return null;
      }
      const outName =
      ...
      await queryRows(
        ...
        sql`
          ...
          SET download_object_path = ${objectPath},
              ...
              status = CASE WHEN status = 'generated_download_failed' THEN
                CASE WHEN EXISTS (
                  SELECT 1 FROM document_generation_job_items
                  WHERE job_id = ${args.jobId} AND firm_id = ${args.firmId} AND status = 'failed'
                  LIMIT 1
      ...
      const code = classifyDocGenError(
        (e as any)?.code ?? null,
        action === "print" ? "storage_write" : "zip_build",
        msg,
      );
      const hr =
        code === "UNKNOWN"
          ? `Print packaging failed (technical code: ${code}).`
          : DOC_GEN_HUMAN_MESSAGE[code];
      persistDownloadFailure(code, hr);
      return null;
    }
  }
  ...
  try {
    ...
    const selectedTemplateRefs = (() => {
      ...
      return raw
        .map((x) => {
          const r0 = x && typeof x === "object" ? (x as any) : null;
          const source =
            r0?.source === "master" ? ("master" as const) : ("firm" as const);
          const id =
            typeof r0?.id === "number"
              ? r0.id
              : Number.parseInt(String(r0?.id ?? ""), 10);
    ...
    for (let i = 0; i < selectedCaseIds.length; i++) {
      ...
      const base0 =
        normalizeZipFolderBase(purchaser1Raw) ||
        normalizeZipFolderBase(caseRefRaw) ||
        `case-${caseId}`;
      let base = base0;
    ...
    const sorted = [...successItems].sort((a, b) => {
    ...
    const entries: Array<{ zipPath: string; objectPath: string }> = [];
    for (const it of sorted) {
      ...
      const caseFolder =
        caseFolderById.get(caseId) ?? `${String(1).padStart(2, "0")}_case-${caseId}`;
      ...
      const baseName = safeFilenameAscii(`${templateName}.${outputExt}`) || `document.${outputExt}`;
      const fileBase = `${String(templateOrder + 1).padStart(2, "0")}_${baseName}`;
      entries.push({
        zipPath: `${rootPrefix}${caseFolder}/${fileBase}`,
        objectPath: op,
      });
    }
    ...
    if (!zipBytes || zipBytes.length === 0) {
      persistDownloadFailure("ZIP_BUILD_FAILED", DOC_GEN_HUMAN_MESSAGE.ZIP_BUILD_FAILED);
      return null;
    }
    const zipFileName =
      safeFilenameAscii(
        `Document_Automation_${new Date().toISOString().slice(0, 10)}.zip`,
      ) || `document-automation-${args.jobId}.zip`;
    const objectPath = `/objects/temp-generated/${args.firmId}/document-generation-jobs/${args.jobId}.zip`;
      ...
      logger.info(
        {
          ...
          finalize_zip_ms: Date.now() - startedAt,
        },
...
async function processAutomationGenerationJobStep(
  ...
  timing.claim_item_ms = Date.now() - claimStartedAt;
  ...
  try {
    ...
    if (templateSource === "master") {
      ...
      {
        const updateItemStartedAt = Date.now();
        const setParts: Array<ReturnType<typeof sql>> = [
          sql`status = 'success'`,
        ...
        timing.update_item_ms = Date.now() - updateItemStartedAt;
      }
      {
        const countsStartedAt = Date.now();
        await updateJobCounts(r, args);
        timing.update_counts_ms = Date.now() - countsStartedAt;
      }
      {
        const finalizeStartedAt = Date.now();
        await finalizeDocGenJobIfDone(r, args);
        timing.finalize_ms = Date.now() - finalizeStartedAt;
      }
      await touchJobHeartbeat(r, args, caps);
      try {
        const progress = await computeDocGenJobProgress(r, args);
    ...
    try {
      ...
      if (action === "print") {
        ...
        await writeAuditLog({
          ...
          actorId: req.userId,
          actorType: req.userType,
          action: "documents.generation_jobs.finalize",
          entityType: "document_generation_job",
          entityId: undefined,
          detail: `jobId=${jobId}`,
          ipAddress: req.ip,
...
router.get(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    if (st === "generated_download_failed") {
      res.json({
        ok: true,
        jobId,
...
router.get(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    const canDownload =
      status === "generated_download_failed"
        ? (progress.total > 0 && progress.success > 0)
    ...
    const failedItemRows = progress.failed > 0 || status === "generated_download_failed"
      ? await queryRows(
          r,
          sql`
            SELECT
            ...
            LEFT JOIN cases c ON c.id = i.case_id AND c.firm_id = i.firm_id
            WHERE i.job_id = ${jobId} AND i.firm_id = ${req.firmId!}
              AND (i.status = 'failed' OR i.error_code IS NOT NULL)
            ORDER BY i.id ASC
          `,
        )
      : [];
    const failedItems = failedItemRows.map((fi) => {
      const rawCode = typeof fi.error_code === "string" ? fi.error_code : null;
      const rawMsg = typeof fi.error_message === "string" ? fi.error_message : "";
      const diag = typeof fi.diagnostic === "string" || (fi.diagnostic != null && typeof fi.diagnostic === "object")
        ? fi.diagnostic
        : null;
      let phase = "generation";
      try {
        const dObj = typeof diag === "string" ? JSON.parse(diag) : diag;
        if (dObj && typeof dObj === "object") {
          const p = typeof (dObj as any).phase === "string" ? (dObj as any).phase : null;
          if (p) phase = p;
        }
      } catch {}
      if (typeof (fi as any).diagnosticRaw == "string") {
        try {
          const dObj = JSON.parse((fi as any).diagnosticRaw);
          if (dObj && typeof dObj === "object") {
            const p = typeof (dObj as any).phase === "string" ? (dObj as any).phase : null;
            if (p) phase = p;
          }
      ...
      const code: DocGenErrorCode = classifyDocGenError(
        rawCode,
        phase,
        rawMsg,
      );
      const humanMessage = code === "UNKNOWN" && rawMsg
        ? `Generation failed. Technical error code: ${rawCode ?? code}`
        : DOC_GEN_HUMAN_MESSAGE[code];
      return {
        itemId: typeof fi.id === "number" ? fi.id : String(fi.id ?? ""),
        caseId: typeof fi.case_id === "number" ? fi.case_id : null,
        templateId: typeof fi.template_id === "number" ? fi.template_id : null,
        caseReference: typeof fi.case_reference === "string" ? fi.case_reference : null,
        templateName: typeof fi.template_name === "string" ? fi.template_name : null,
        phase,
        errorCode: code,
    ...
    res.status(200).json({
      ...
      job: jobPayload,
      failedItems,
      ...(status === "failed" || status === "generated_download_failed"
        ? {
            error: {
              code:
                typeof job.error_code === "string" ? String(job.error_code) : status === "generated_download_failed" ? "ZIP_BUILD_FAILED" : "GENERATION_FAILED",
              message:
                typeof job.error_summary === "string"
                  ? String(job.error_summary)
                  : status === "generated_download_failed"
...
router.get(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    const typedItems = items.map((it) => {
      const rawCode = typeof (it as any).error_code === "string" ? (it as any).error_code : null;
      const rawMsg = typeof (it as any).error_message === "string" ? (it as any).error_message : "";
      const diag = typeof (it as any).diagnostic === "string" || ((it as any).diagnostic != null && typeof (it as any).diagnostic === "object")
        ? (it as any).diagnostic
        : null;
      let phase = "generation";
      try {
        const dObj = typeof diag === "string" ? JSON.parse(diag) : diag;
      ...
      const code: DocGenErrorCode = classifyDocGenError(rawCode, phase, rawMsg);
      const humanMessage = code === "UNKNOWN" && rawMsg
        ? `Generation failed. Technical error code: ${rawCode ?? code}`
        : DOC_GEN_HUMAN_MESSAGE[code];
      return {
        ...it,
        templateName: typeof (it as any).template_name === "string" ? (it as any).template_name : null,
        caseReference: typeof (it as any).case_reference === "string" ? (it as any).case_reference : null,
        phase,
        errorCode: String((it as any).status) === "failed" ? code : null,
        errorMessage: String((it as any).status) === "failed" ? humanMessage : null,
...
router.get(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    const jobs = await queryRows(
      r,
      sql`SELECT status, action, config, case_ids FROM document_generation_jobs WHERE id = ${jobId} AND firm_id = ${req.firmId!} LIMIT 1`,
    );
    const job = jobs[0] as any;
    if (!job) {
      fail(404, "JOB_NOT_FOUND", "Job not found", null, false);
    ...
    const done =
      ...
      progress.success + progress.failed === progress.total;
    if (!done) {
      fail(409, "JOB_NOT_READY_FOR_DOWNLOAD", "Job is not ready for download yet", { progress }, true);
      return;
    }
    ...
    const jobConfig =
      job?.config && typeof job.config === "object"
        ...
        : {};
    const outputExt =
      (jobConfig as any).outputFormat === "pdf"
        ? "pdf"
        : (jobConfig as any).outputFormat === "docx"
          ? "docx"
          : "pdf";
    ...
    const selectedTemplateRefs = (() => {
      ...
      return raw
        .map((x) => {
          const r0 = x && typeof x === "object" ? (x as any) : null;
          const source = r0?.source === "master" ? ("master" as const) : ("firm" as const);
          const id =
            typeof r0?.id === "number" ? r0.id : Number.parseInt(String(r0?.id ?? ""), 10);
          return Number.isFinite(id) && id > 0 ? { source, id: Math.trunc(id) } : null;
        })
        .filter((x): x is { source: "firm" | "master"; id: number } => Boolean(x));
    })();
    ...
    if (selectedCaseIds.length > 0) {
      ...
      for (const row of rows3) {
        ...
        if (!Number.isFinite(caseId) || caseId <= 0) continue;
        if (purchaser1NameByCaseId.has(Math.trunc(caseId))) continue;
        const name = typeof (row as any).name === "string" ? String((row as any).name) : "";
        if (name.trim()) purchaser1NameByCaseId.set(Math.trunc(caseId), name.trim());
      }
    }
    ...
    const rootFolder = safeZipSegment(`Document_Automation_${new Date().toISOString().slice(0, 10)}`);
    const rootPrefix = rootFolder ? `${rootFolder}/` : "";
    const zipFileName = safeFilenameAscii(`${rootFolder}.zip`) || `document-automation-${jobId}.zip`;
    ...
    const files: ManifestFile[] = sorted.map((it) => {
      const caseId = Number((it as any).case_id ?? NaN);
      ...
      const templateKey = `${templateSource}:${templateId}`;
      const templateIndex = selectedTemplateIndexByKey.get(templateKey);
      const templateOrder = typeof templateIndex === "number" && Number.isFinite(templateIndex) ? templateIndex : 0;
      const templateName =
      ...
      const baseName = safeFilenameAscii(`${templateName}.${outputExt}`) || `document.${outputExt}`;
      const fileBase = `${String(templateOrder + 1).padStart(2, "0")}_${baseName}`;
      const zipPath = `${rootPrefix}${caseFolder}/${fileBase}`;
      const folderPath = `${rootPrefix}${caseFolder}`.replace(/\/+$/g, "");
      const op = typeof (it as any).object_path === "string" ? String((it as any).object_path) : "";
      return {
        itemId:
          typeof (it as any).id === "number"
            ? Number((it as any).id)
            : Number.parseInt(String((it as any).id ?? "0"), 10) || 0,
        status: String((it as any).status ?? ""),
        fileName: fileBase,
        folderPath,
        zipPath,
    ...
    const ttlSec = 10 * 60;
    const toSign = files.filter((f) => f.status === "success" && f.storagePath);
    let cursor = 0;
    const workerCount = Math.max(1, Math.min(5, toSign.length));
    const workers = Array.from({ length: workerCount }).map(async () => {
      while (cursor < toSign.length) {
        const i = cursor++;
        const f = toSign[i]!;
        try {
          f.signedUrl = await supabaseStorage.createSignedDownloadUrl(String(f.storagePath), ttlSec);
        } catch {
          f.signedUrl = null;
        }
      }
...
router.get(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    const forceRaw = one((req.query as any).force);
    const forceRebuild = forceRaw === "1" || forceRaw === "true";
    if (forceRebuild) {
      try {
        await queryRows(
          r,
      ...
      (job as any).download_object_path = null;
      (job as any).download_file_name = null;
      (job as any).download_mime_type = null;
      try {
        logger.info(
          {
            event: "docgen_download_force_rebuild_requested",
            firmId: req.firmId ?? null,
            userId: req.userId ?? null,
            jobId,
            priorStatus: st,
          ...
          "[documents] download force rebuild",
        );
      } catch {}
    }
    if (st === "finalizing") {
    ...
    if (st !== "completed" && st !== "completed_with_errors" && st !== "generated_download_failed") {
      const progress = await computeDocGenJobProgress(r, {
        firmId: req.firmId!,
        jobId,
      });
    ...
    try {
      ...
      if (!objectPath) {
        const packaged = await ensureDocGenJobDownloadObject(r, {
          firmId: req.firmId!,
          jobId,
        });
        if (!packaged) {
          ...
          fail(
            409,
            "ZIP_PACKAGING_FAILED",
            "Download package build failed. Status may have transitioned to generated_download_failed; use Retry Download button to attempt packaging again (no document regeneration).",
            { status: st, progress, retry: true },
            true,
          );
          return;
        }
        objectPath = packaged.downloadObjectPath;
        fileName = packaged.downloadFileName;
        fallbackContentType = packaged.downloadMimeType;
      }
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    if (st !== "completed" && st !== "completed_with_errors" && st !== "generated_download_failed") {
      ...
      fail(
        409,
        "JOB_NOT_COMPLETED",
        "Job is not in a state eligible for download re-packaging.",
        { status: st || "pending", progress },
        true,
      );
      return;
    ...
    try {
      ...
      if (!packaged) {
        ...
        fail(
          409,
          "ZIP_PACKAGING_FAILED",
          "Download package build failed. Use GET /documents/jobs/:jobId/download?force=true again to retry packaging; no document regeneration.",
          { status: st, progress, retry: true },
          true,
        );
        return;
      ...
      try {
        await writeDocumentGenerationLog(r, {
          ...
          actionType: "DOCUMENT_ZIP_CREATED",
          jobId,
          fileNames: [packaged.downloadFileName],
          ipAddress: req.ip,
          userAgent: req.headers["user-agent"],
          requestId,
          caseIds: Array.isArray((job as any).case_ids)
            ? (job as any).case_ids
                .map((x: any) => typeof x === "number" ? x : Number.parseInt(String(x ?? ""), 10))
                .filter((x: any) => Number.isFinite(x))
                .map((x: any) => Math.trunc(x))
                .filter((x: any) => x > 0)
            : [],
        });
      } catch {}
      res.status(200).json({
        ok: true,
        jobId,
        status: String((job as any).status ?? "completed"),
        downloadUrl: `/documents/jobs/${jobId}/download`,
        downloadFileName: packaged.downloadFileName,
        downloadMimeType: packaged.downloadMimeType,
        regenerated: false,
      ...
      logger.error(
        { err, path: req.path, firmId: req.firmId, userId: req.userId, jobId },
        "[documents] generation_job_repack_download_failed",
      );
      res.status(500).json({
        ok: false,
        error: { code: "INTERNAL", message: "Internal Server Error", details: null, retryable: true },
        meta: {
          request_id: requestId ?? null,
          timestamp: new Date().toISOString(),
          duration_ms: Date.now() - startedAt,
...
export {
  ...
  renderPdfMappedTemplate,
  renderPdfTextBoxMappedTemplate,
  computeDocGenJobProgress,
  finalizeDocGenJobIfDone,
  classifyDocGenError,
  DOC_GEN_HUMAN_MESSAGE,
  writeDocumentGenerationLog,
  ensureDocGenJobDownloadObject,
};
